...
<script type="text/javascript">
	...
	function fInitFormLec(object) {
		...
		if( object == "" || object == null || object == undefined) {
			...
			$("#delCourseno").val("");
			$("#action").val("I");
			$("#sel_lec option:eq(0)").prop("selected", true);
			...
			//강의정보 불러온거면 I 아니면 U
			$("#action").val()=='S'? $("#action").val("I"):$("#action").val("U");
		}		
		// 모달 팝업
		gfModalPop("#layer1");
	}
	...
	//강의정보 불러오기
	function flec_info(){
		var lec_id= $("#sel_lec").val();
		fLecSel(lec_id);
		var action = $("#action").val("S");
	}
						...
						<div class="div_lec_list" id="div_lec_list">
							<table class="col">
								...
								<tbody id="listLecture">
									...
									<tr>
									    <td>{{ row.lec_id }}</td>
										<td><a @click="b_stdList(row.lec_id)" >{{ row.lec_name }}</a></td>
										<td>{{ row.name }}</td>
				...
				<table class="row">
					...
					<tbody>
						<tr id="load">
								...
								<td colspan="4">
									<select id="sel_lec" value="" onchange="flec_info()" class="inputTxt p100">
