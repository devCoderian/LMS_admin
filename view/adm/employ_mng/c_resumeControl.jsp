...
<script type="text/javascript">
	...
	function email_rpstt_check(loginId){
		...
		callAjax("/adm/c_email_rpstt_check.do", "post", "text", true, param,
				resultCallback1);
	}
	// 이메일 전송 확인
	var std_Mail_List = [];
	...
	function email_send_check(loginId) {
		...
		callAjax("/adm/c_resumeControl_Check.do", "post", "text", true, param,
				resultCallback1);
	}
	// 이메일 전송
	function email_send(loginId) {
		...
		callAjax("/adm/c_email_send.do", "post", "json", true, param,
				resultCallback1);
	}
	...
	function rs_download(rs_url) {
		...
		jQuery(
				"<form action='/adm/c_download.do' method='post'>" + params
