					...
					<div class="content">
						...
						<p class="Location">
							<a href="#" class="btn_set home">메인으로</a> <a href="#"
								class="btn_nav">학습 관리</a> <span class="btn_nav bold">취업 정보 관리</span> <a href="javascript:location.reload();" class="btn_set refresh">새로고침</a>
						</p>
							...
							<table class="col">
								...
								<tbody id="liststudent">
									<template v-for="(row, index) in items"  v-if="items.length">
										<tr>
											<td>{{ row.std_num }}</td>
											<td><a @click="b_profile(row.loginID)" >{{ row.name }}</td>
											<td>{{ row.tel }}</td>
							...
							<table class="col">
								...
								<tbody id="liststudent">
									<template v-for="(row, index) in items"  v-if="items.length">
										<tr>
											<td>{{ row.std_num }}</td>
											<td><a @click="b_profile(row.loginID)" >{{ row.name }}</a></td>
											<td>{{ row.tel}}</td>
											<td>{{ row.lec_name }}</td>
											<td>{{ row.join_date }}</td>
											<td align="center">
               ...
               <table class="row">
                 ...
                 <tr>
                        <td colspan="6">
                         ...
                         <%-- 	 <c:if test="${list_lec == null}">
                         	<tr><td colspan="6">수강정보가 없습니다</td></tr>
                               </c:if>
                               <c:if test="${list_lec != 0}">   --%>
