								...
								<table class="col">
									...
									<tbody id="lec_List">
										<template v-for="(row,index) in items" v-if="items.length">
											<tr>
												<td class="cursorArea"><a @click="selChart(row.lec_id)"><strong>{{row.lec_name}}</strong></a></td>
												<td>{{row.startdate}}</td>
												<td>{{row.enddate}}</td>
