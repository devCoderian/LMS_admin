		...
		vm = new Vue({
			...
			//<script>="text/javascript" src="//rawgit.com/wenzhixin/vue-bootstrap-table/develop/docs/static/dist/vue-bootstrap-table.js">
			  ...
			  methods:{		
				  ...
				  equipDetail:function(equ_name){
					console.log("하나!: " + equ_name);  
			  		fequipDetail(equ_name);
				  },
 ...
 */
	...
	function fListequipResult(data, currentPage) {
 	...
 	//장비 상세조회, 받는 순서 주의!! equ_id가 먼저 들어오자!
 	function fequipDetail(equ_name){
 		
 		console.log("둘!!:" + equ_name);
 		
 	/* 	currentPage = currentPage || 1;
 		console.log("지금 페이지!: " + currentPage); */
 		
 		param = {
 				 equ_name : equ_name
 		}

 		console.log("파람 속 아이디!!:" + param.equ_name);

 		var resultCallback = function(data) {
 			
 			console.log("셋!! : " + data);
 			fequipDetailResult(data);
 		}
 		
		callAjax("/adm/equipDetail.do","post","json",true,param,resultCallback);
 	}
 	
 	
 	//장비 상세조회 콜백
 	function fequipDetailResult(data) {
 	
 		edvm.items = [];
		edvm.items = data.equipDetail;
...
<!-- 									<tr v-on::click="rowClicked(this)">
									    ...
									    <td class="cursorArea"><a @click="equipDetail(row.equ_name)"><strong>{{ row.equ_name }}</strong></a></td>
										<td>{{ row.equ_num }}</td>
										<td>{{ row.equ_note }}</td>
									</tr>
									</template>
	...
	<div id="layer2" class="layerPop layerType2" style="width : 1000px;">
		<dl>
			...
			<dd class="content">	
				...
				<div class="btn_areaC mt30">
				<a href=""	class="btnType gray"  id="btnCloseEquip" name="btn"><span>확인</span></a>
			</div>
			</dd>
		</dl>
	</div>
