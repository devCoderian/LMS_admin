...
<script>
	...
	$(document).on("click", ".open-testSend", function () {
		var Id = $(this).data('id');
		$(".modal-body #send_testId").val(Id);
		gfModalPop("#layer2");
		});
	...
	$( function() { $( "#test_start1" ).datepicker({dateFormat:"yymmdd"}); }).val();	
	$( function() { $( "#test_end1" ).datepicker({dateFormat:"yymmdd"}); }).val();
	/* $("input[type=radio][name=testreg]").on('click',function(){
		
		var chkval = $("input[type=radio][name=testreg]:checked").val();
		
		if(chkval == "본시험"){
		}
		else if(chkval== "재시험"){
			var t = document.getElementById("select");
			var tIndex = document.getElementById("select").options.selectedIndex;
			var selva = t.options[tIndex].value;
		}
	}) */
	})
	...
	function init1(){
		...
		var name = document.getElementById("test_name1").value = null;
		var start = document.getElementById("test_start1").value=null;
		var end = document.getElementById("test_end1").value=null;
	}
				...
				$.ajax({
					...
					,success : function(resp){
							...
							$.each(resp,function(index,item){
								d+= "<tr>"+"<td>"+(index+1)+"</td>";
								d+= "<td>"+'<a href="examRegist?test_id='+item.test_id+'">';
								d+= "<strong>"+item.test_name+"</strong></a>"+"</td>";
								d+= "<td>"+item.test_sort+"</td>";
								d+= "<td>"+item.test_start+"</td>";
								d+= "<td>"+item.test_end+"</td>";
								if(item.test_sort=='본시험'){d+="<td>"+"<a class='open-testSend btn btn-primary'data-id='"+item.test_id+"'href='#addBookDialog'>"+"<strong>재시험 등록</strong></a>"+"</td>"+"</tr>"}
								else{d+="<td>"+"</td>"+"</tr>"};
							})
							$("#lec_List").html(d);
						}
...
/* 		 var tname = document.getElementById("test_name").value;
	 ...
	 function reTest2(){
		 var tname1 = document.getElementById("test_name1").value;
		 var tstart1 = document.getElementById("test_start1").value;
		 var tend1 = document.getElementById("test_end1").value;
		 var target = document.getElementById("select");
	     var res = target.options[target.selectedIndex].value;
	     var id = $("#send_testId").val();
	     var staArr = tstart1.split('.');
		 var endArr = tend1.split('.');
...
<body>
                      ...
                      <select id="select" name="select" style="width: 100px;">
                       ...
                       <a class="btnType blue" href="javascript:modalGo();" name="modal"><span>시험등록</span></a>
                        ...
                        <table class="col">
                           <thead>
                              <tr>
                                 <th scope="col" colspan="6">시험 목록</th>
                              </tr>
                           </thead>
   ...
   <input type="hidden" id="send_lecId" name="lec_id" value="">
		<dl>
			...
			<dd class="content">
				...
				<table class="row">
					...
					<tbody>
						<!-- <tr>
						<input type="radio" id="tereg" name="testreg" value="본시험" checked>본시험
						<input type="radio" id="retest" name="testreg" value="재시험" >재시험<br>
						</tr> -->
						<tr>
						...
						<!--<tr>
							<th scope="row">시험 목록 <span class="font_red">*</span></th>
							<td colspan="3">
		...
		<a href="" onclick="clpop1()" class="closePop"><span class="hidden">닫기</span></a>
	</div>
		</form>
		
		<form id=frm>
   <div id="layer2" class="layerPop layerType2" style="width: 450px;">
   <input type="hidden" id="send_lecId" name="lec_id" value="">
   <div class="modal-body">
   <input type="hidden" id="send_testId" name="send_testId" value="">
   </div>
		<dl>
			<dt>
				<strong style="width:400px; height:100px;">시험 등록</strong>
			...
			<dd class="content">
				...
				<table class="row">
					...
					<tbody>
						<tr>
							<th scope="row">시험명 <span class="font_red">*</span></th>
							<td><input type="text" class="inputTxt p100" name="test_name1" id="test_name1" /></td>
						</tr>
						<tr>
							<th scope="row">시작날짜 <span class="font_red">*</span></th>
							<td colspan="3"><input type="text" class="inputTxt p100"
								name="test_start1" id="test_start1" data-date-format='yyyy.mm.dd' readonly/></td>
						</tr>
						<tr>
							<th scope="row">마감날짜<span class="font_red">*</span></th>
							<td colspan="3"><input type="text" class="inputTxt p100"
								name="test_end1" id="test_end1" data-date-format='yyyy.mm.dd' readonly/></td>
						</tr>
					</tbody>
				</table>
				<div class="btn_areaC mt30">
					<a href="javascript:reTest2();" class="btnType blue" id="RegisterCom" name="btn"><span>등록</span></a> 
					<a href="javascript:cancelAll();"	class="btnType gray"  id="btnCloseGrpCod" name="btn"><span>모든 내용 삭제</span></a>
				</div>
			</dd>
		</dl>
		<a href="" onclick="clpop2()" class="closePop"><span class="hidden">닫기</span></a>
	</div>
		</form>
