...
/*여기까지 공통 코드------------------------------------------------------------------------------------------------ */
	...
	function loadTestResult(){
		var target = document.getElementById("lec_id2");
     	var res = target.options[target.selectedIndex].value;
     	var TestId = document.getElementById("test_id3").value;
		 $.ajax({
	        url : "loadTest",
	        type : "POST",
	        data : {'test_id':res,
...
<body>
 <form id="myForm">
               ...
               <li class="contents">
                  ...
                  <div class="content">
                     ...
                     <p class="conTitle">              
                      <span>시험 문제 관리</span>
                      <span class="fr">
                      <a class="btnType blue" href="javascript:goBack();" name="backpage"><span>뒤로가기</span></a>
                       <a class="btnType blue" href="javascript:modalGo();" name="modal"><span>문제등록</span></a>
                       <a class="btnType blue" href="javascript:loadTest();" name="modal"><span>이전 시험 불러오기</span></a>
                      </span>
					 </p>
                        <table class="col">
                           <thead>
   ...
   <input type="hidden" id="send_testId" name="send_testId" value="">
		...
		<dl>
			<dt>
				<strong style="width:400px; height:100px;">이전 시험 불러오기</strong>
			</dt> 
			<dd class="content">
				<!-- s : 여기에 내용입력 -->
				<table class="row">
					...
					<tbody>
						<tr>
							...
							<td>
							<input type="hidden" id="test_id3" name="test_id3" value="${test_id}">
							<select id="lec_id2" value="" class="inputTxt p100">
								<option>시험명 선택</option>
								<c:forEach items="${test_List}" var="list">
								<option value="${list.test_id}">${list.test_name}</option>
								</c:forEach>
								</select>
							</td>
						</tr>
				...
				<div class="btn_areaC mt30">
					<a href="javascript:loadTestResult();" class="btnType blue" id="RegisterCom" name="btn"><span>등록</span></a> 
				</div>
			</dd>
		</dl>
		<a href="" onclick="clpop()" class="closePop"><span class="hidden">닫기</span></a>
