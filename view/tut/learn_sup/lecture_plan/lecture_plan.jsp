...
<script type="text/javascript">
	...
	function fInit(object) {
		...
		if (object == "" || object == null) {
			...
			$("#lecrm").eq(0).val(object.lecrm_id);
	...
	function flec_info_result(object,week) {
		...
		$("#tel").val(object.tel);
		$("#mail").val(object.mail);
		//$("#lecrm").val(object.lecrm_id);
		//$("#lecrm").text(object.lecrm_name);
		$("#lec_goal").val(object.lec_goal);
		$("#atd_plan").val(object.atd_plan);
...
<body>
	<form id="myForm" action="" method="">
		<input type="hidden" id="currentPage" value="1"><input
			type="hidden" id="tmp_lec"  name="lec_id"  value="">  <input type="hidden"
			name="action" id="action" value="">
			<input type="hidden"
			name="progress" id="progress" value="">
							...
							<p class="conTitle mt50">
								...
								<input type="text" id="searchWord" name="searchWord"
									...
									href="javascript:flist_lec()" name="search"><span
										...
										<a class="btnType blue" href="javascript:fPopModal();" name="modal">
										<span>계획서 등록</span></a>
								</span>
							</p>
			...
			<dl>
				...
				<dd class="content">
					<table class="row" id="edit_table" name="edit_table">
						...
						<tbody>
							<tr id="load">
								<th scope="row">불러오기</th>
								<td colspan="4">
									<select id="lec_id" value="" onchange="flec_info()" class="inputTxt p100">
							...
							<tr>
								<th scope="row">과목</th>
								<td>
									<select value=""  id="lec_sel" onchange="flec_sel()">
										<option>과정명 선택</option>
										<c:forEach items="${mlist_lec}" var="list">
											<option value="${list.lec_id}">${list.lec_name}</option>
										</c:forEach> 
							...
							<tr>
								...
								<td><!-- <input type="text" name="lecrm" id="lecrm"
									...
									<select name="lecrm_id" id="lecrm">
										<c:forEach items="${mlist_room}" var="list">
