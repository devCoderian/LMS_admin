	...
	$(document).ready(function(){
		...
		$.ajax({
			...
			success:function(data){
				...
				html +='<input type="text" name="titlename" value="'+titlename+'">';
				
				for(var i=0; i<totalCount; i++) {
					html += '<input type="text" name="placeList['+i+'].cno" value="'+cno+'">';
					html += '<input type="text" name="placeList['+i+'].contentId" class="contentId'+i+'" >';
				}
				html += '<input type="submit" value="선택"></td></form>';
							...
							$.ajax({
								...
								success:function(data){
									...
									$(data).find('item').each(function() {
										...
										markerPosition = new kakao.maps.LatLng(mapx[subnum], mapy[subnum]);
		...
		$.ajax({
			...
			success:function(data){
				...
				$(data).find('item').each(function(i){
					...
					$("#a"+i).attr("href", "/detail?contentid="+contentid+"&mapx="+mapx+"&mapy="+mapy+"&areacode="+areacode+"&sigungucode="+sigungucode+"&title="+encodeURIComponent(title));
