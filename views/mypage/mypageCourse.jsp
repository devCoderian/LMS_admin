...
<body>
											...
											<c:forEach items="${myList}" var="myList">
												<tr>
													<td>
														...
														<a
														href="/mypage/mypageCourseUpdate?writer=${member.userId }&cno=${myList.cno}" style="color: black;"><c:out
																value="${myList.title}" /></a>
													</td>
													<td><c:out value="${myList.tema}" /></td>
													<td><c:out value="${myList.schedule}" /></td>
												</tr>
											</c:forEach>
										</table>
											...
											<div class="col-xs-10 col-sm-10">
												<div class="input-group">
													<input type="text" name="keyword" id="keywordInput"
														...
														<c:if test="${member != null}">
															<button type="button" class="btn btn-primary" data-toggle="modal"  data-target="#myModal">코스 생성하기</button>
														</c:if>
													</span>
												</div>
											</div>
											...
											<ul class="pagination">
												...
												<c:forEach begin="${pageMaker.startPage}"
													...
													<li
														...
														<a class="page-link" href="mypageCourse${pageMaker.makeSearch(idx)}">${idx}</a>
													</li>
												</c:forEach>
			...
			<div class="modal-dialog" role="document">
				<div class="modal-content">
					<div class="modal-header">
						<h4 class="modal-title" id="myModalLabel">코스 생성하기</h4>
					</div>
					
						<form action="/course/csWrite" method="post">
	...
	<script type="text/javascript">
		...
		$(function() {
			$('#searchBtn').click(
					function() {
						self.location = "mypageCourse" + '${pageMaker.makeQuery(1)}'
								+ "&searchType="
